...
layers = []
aucs = []
for num_layer in range(10,61, 5):
    layers.append(num_layer)
    dnn = Sequential()
    dnn.add(Dense(num_layer, input_dim=n_dims, activation='relu'))
    dnn.add(Dense(num_layer, input_dim=n_dims, activation='relu'))
    ...
    aucs.append(auc)
plt.grid()
plt.scatter(layers,aucs,label="AUC",color='black',zorder=2)
plt.legend()
plt.xlabel("Number of nodes")
plt.xticks(layers)
plt.title("AUC vs Number of nodes in two hidden layers; fixed epoch=10")
plt.savefig(path+"auc_vs_nodes.pdf",dpi=200)
