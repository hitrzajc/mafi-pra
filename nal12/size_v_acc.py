...
def split_xy(rawdata):
    ...
    data_x=rawdata.iloc[:, 1:]  # features only, drop the first column which is 'hlabel'
...
acc = []
x_os = []
sizes = [10**i for i in range(2,7)] #7
...
for size in sizes:
    ...
    print("Model {} trained successfully.".format(size))
...
plt.grid()
plt.scatter(sizes, acc, label='Accuracy',color='black', zorder=2)
...
plt.savefig(path+'model_accuracy_vs_data_size.pdf', dpi=200)