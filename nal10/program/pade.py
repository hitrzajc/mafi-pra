...
for i in range(1,len(sol)):
    ...
    #     vec = np.dot(Matrices[j], sol[i])
    #     sol[i] = LA.solve(MatricesC[j], vec)
...
plt.xlabel("x")
plt.ylabel("t")
plt.title("Absoulte error of the Pade method with N=8 dieriative")
plt.savefig(PATH+"abs_error_pade.pdf")